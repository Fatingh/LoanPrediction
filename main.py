...
if uploaded_file:
    ...
    if page == "Model Training":
        ...
        st.markdown("The target variable is fixed to `loan_status`. Click the button below to train the model.")
        ...
        if st.button("Train Model"):
            ...
            # Save model and training columns to session state
            st.session_state['model'] = model
            st.session_state['training_columns'] = X_train.columns

        # Prediction Form
        st.subheader("Prediction Form")
        st.markdown("Fill in the form below to predict whether a loan application will be approved.")

        if 'model' in st.session_state and 'training_columns' in st.session_state:
            with st.form(key="prediction_form"):
                # Dynamically generate inputs for all columns in the dataset
                user_inputs = {}
                for column in df.drop(columns=["loan_status"]).columns:
                    if df[column].dtype == 'object':  # Categorical input
                        user_inputs[column] = st.selectbox(column, df[column].unique())
                    else:  # Numerical input
                        user_inputs[column] = st.number_input(column, float(df[column].min()), float(df[column].max()))
                ...
                if submit_button:
                    ...
                    # Ensure input data matches the training data structure
                    input_data = input_data.reindex(columns=st.session_state['training_columns'], fill_value=0)
                    ...
                    st.write(f"The predicted loan status is: **{result}**")
        else:
